...
- `readFailure` in `src/core/files.ts` casts a caught `unknown` to read `.code`; a type guard would say the same thing without the cast.
...
- `promptTokensFor` throws away the filesystem cause (ENOENT reads the same as EACCES), and builds its path with `join(forgeDir, "..", promptFile)`, which mangles an absolute `prompt_file`; `resolve` would not.
